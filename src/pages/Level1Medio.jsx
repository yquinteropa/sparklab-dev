  ...
  if(screen==='intro') return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'2rem 1rem',minHeight:400}}>
      <div style={{background:'hsl(222, 47%, 11%)',border:'0.5px solid #e2e8f0',borderRadius:12,padding:'26px 22px',maxWidth:440,width:'100%'}}>
        ...
        <p style={{fontSize:13,color:'hsl(215, 20%, 70%)',lineHeight:1.65,marginBottom:12}}>Las resistencias tienen bandas de color que indican su valor. Aprenderás a leer y calcular ese valor en dos partes.</p>
        <div style={{background:'hsl(217, 33%, 17%)',borderRadius:8,padding:'10px 14px',fontSize:12,color:'hsl(215, 20%, 70%)',lineHeight:1.6,marginBottom:16}}>
          <strong style={{fontWeight:500,color:'hsl(210, 40%, 96%)'}}>Fórmula (4 bandas):</strong><br/>
        ...
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
          <button onClick={()=>setScreen('book')} style={{padding:'8px 16px',borderRadius:8,border:'0.5px solid #e2e8f0',background:'hsl(217, 33%, 17%)',fontSize:13,cursor:'pointer',color:'hsl(215, 20%, 75%)'}}>📖 Tabla de colores</button>
          <button onClick={()=>setScreen('game')} style={{padding:'8px 20px',borderRadius:8,border:'0.5px solid #93c5fd',background:'hsl(217, 91%, 20%)',fontSize:13,cursor:'pointer',color:'hsl(199, 89%, 70%)',fontWeight:500}}>▶ Empezar</button>
        </div>
      </div>
    </div>
  );
  ...
  if(screen==='book') return (
    <div style={{padding:'1.5rem 0',maxWidth:560,margin:'0 auto'}}>
      ...
      <div style={{marginTop:12,fontSize:12,color:'hsl(215, 20%, 70%)',lineHeight:1.65}}>
        <strong style={{fontWeight:500,color:'hsl(210, 40%, 96%)'}}>Cómo calcular:</strong> Toma el dígito de banda 1, el de banda 2, multiplica por el multiplicador de banda 3. Banda 4 = tolerancia.
      </div>
      <div style={{display:'flex',justifyContent:'flex-end',marginTop:14}}>
        <button onClick={()=>setScreen('game')} style={{padding:'8px 20px',borderRadius:8,border:'0.5px solid #93c5fd',background:'hsl(217, 91%, 20%)',fontSize:13,cursor:'pointer',color:'hsl(199, 89%, 70%)',fontWeight:500}}>Entendido — ¡a practicar!</button>
      </div>
    </div>
  );
    ...
    <div style={{padding:'1.5rem 0',maxWidth:580,margin:'0 auto',fontFamily:'sans-serif'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.2rem',flexWrap:'wrap',gap:8}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <span style={{fontSize:12,color:'hsl(215, 20%, 70%)',background:'hsl(217, 33%, 20%)',border:'0.5px solid #e2e8f0',borderRadius:8,padding:'3px 10px'}}>Código de colores</span>
        </div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        <span style={{fontSize:11,color:'hsl(215, 20%, 65%)',whiteSpace:'nowrap'}}>Progreso</span>
      ...
      <div style={{background:'hsl(222, 47%, 11%)',border:'0.5px solid #e2e8f0',borderRadius:12,padding:'1.25rem',marginBottom:'1rem'}}>
        {part===0 ? (
          <>
            <div style={{fontSize:15,fontWeight:500,marginBottom:4}}>Parte 1 — Valor a colores <span style={{fontSize:11,color:'hsl(215, 20%, 65%)',fontWeight:400}}>({qIdx+1} de 3)</span></div>
            <div style={{fontSize:13,color:'hsl(215, 20%, 70%)',marginBottom:'1rem',lineHeight:1.5}}>Elige las 4 bandas de color que representan el siguiente valor.</div>
            <div style={{background:'hsl(217, 33%, 17%)',borderRadius:8,padding:'10px 16px',textAlign:'center',marginBottom:'1rem'}}>
              ...
              <div style={{fontSize:12,color:'hsl(215, 20%, 70%)'}}>Tolerancia: {q1.tol}</div>
            </div>
          ...
          <>
            <div style={{fontSize:15,fontWeight:500,marginBottom:4}}>Parte 2 — Colores a valor <span style={{fontSize:11,color:'hsl(215, 20%, 65%)',fontWeight:400}}>({qIdx+1} de 3)</span></div>
            <div style={{fontSize:13,color:'hsl(215, 20%, 70%)',marginBottom:'1rem',lineHeight:1.5}}>Lee las bandas de la resistencia y escribe su valor en ohmios.</div>
            ...
            <div style={{display:'flex',justifyContent:'center',gap:10,marginBottom:'1rem',flexWrap:'wrap'}}>
              {[q2.b1,q2.b2,q2.b3,q2.b4].map((lbl,i)=>(
                <div key={i} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                  ...
                  <div style={{fontSize:10,color:'hsl(215, 20%, 65%)'}}>{lbl}</div>
                </div>
              ))}
            </div>
      ...
      <div style={{display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap'}}>
        <button onClick={skip} style={{padding:'7px 18px',borderRadius:8,border:'0.5px solid #e2e8f0',background:'hsl(217, 33%, 17%)',fontSize:12,cursor:'pointer',color:'hsl(215, 20%, 70%)'}}>→ Saltar</button>
        <button onClick={giveHint} style={{padding:'7px 18px',borderRadius:8,border:'0.5px solid #e2e8f0',background:'hsl(217, 33%, 17%)',fontSize:12,cursor:'pointer',color:'hsl(215, 20%, 70%)'}}>💡 Pista</button>
        <button onClick={checkAnswer} style={{padding:'7px 20px',borderRadius:8,border:'0.5px solid #93c5fd',background:'hsl(217, 91%, 20%)',fontSize:12,cursor:'pointer',color:'hsl(199, 89%, 70%)',fontWeight:500}}>✓ Verificar</button>
      </div>
    </div>
  );
}
